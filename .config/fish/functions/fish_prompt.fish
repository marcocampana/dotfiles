function fish_prompt --description 'Write out the prompt'
	...
	# echo -s "  " (set_color $time_color) [(date +%H:%M:%S)] " " $normal "$USER " $normal @ "$__fish_prompt_hostname"
	# echo -n -s →  $normal ' ' (set_color $color_cwd) (prompt_pwd) $normal (set_color $git_prompt_color) (__fish_git_prompt) $normal $prompt_status "$mode_str" " "
